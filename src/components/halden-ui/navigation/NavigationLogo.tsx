...
export function NavigationLogo({
  ...
  const content = (
    <>
      ...
      {label && <span className="hd-label-mono hidden sm:inline">/ {label}</span>}
    </>
  );

  return (
    href.startsWith("/") ? (
      <Link href={href} className={cn("hd-focus-ring flex items-center gap-2.5 rounded-[var(--radius-sm)]", className)} {...props}>
        {content}
      </Link>
    ) : (
      <a href={href} className={cn("hd-focus-ring flex items-center gap-2.5 rounded-[var(--radius-sm)]", className)} {...props}>
        {content}
      </a>
    )
  );
